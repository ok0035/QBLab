...
## Phase 1.5: 썸네일 이미지 검증

커버 이미지가 포함된 포스트가 있으면, 이미지 내용이 포스트 데이터와 일치하는지 확인한다.

### 검증 항목
- cover 필드에 지정된 이미지 파일이 `static/` 하위에 존재하는지 확인
- 이미지를 Read 도구로 열어 눈으로 확인: 가격, 변동률(±N.N%), 포지션, 공포탐욕지수가 포스트 본문과 일치하는지 대조
...
**--change 형식 규칙:**
- 반드시 `+N.N%` 또는 `-N.N%` 형식을 사용한다 (예: `+4.4%`, `-2.5%`)
- 하락일 때는 반드시 `-` 부호를 붙인다
- `down2.5`, `up3.0` 같은 비정규 형식은 사용하지 않는다
- `--change` 옵션에 음수를 전달할 때는 `--change="-2.5%"` 형식(= 사용)으로 전달한다 (argparse가 `-`를 플래그로 오인하는 것을 방지)
